...
async def add_spot_start(message: Message, state: FSMContext):
    ...
    if existing:
        ...
        await message.answer("📍 <b>Выберите адрес</b>:", reply_markup=address_keyboard(), parse_mode="HTML")
        await state.set_state(AddSpotStates.waiting_address)

@router.callback_query(F.data == "new_spot")
async def new_spot(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await callback.message.edit_text("📍 <b>Выберите адрес</b>:", reply_markup=address_keyboard(), parse_mode="HTML")
    await state.set_state(AddSpotStates.waiting_address)
...
@router.callback_query(AddSpotStates.waiting_address, F.data.startswith("address_"))
async def sp_address(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    addr_num = callback.data.replace("address_", "")
    address = f"Адрес {addr_num}"
    await state.update_data(address=address)
    await callback.message.edit_text("🏠 <b>Номер нового места</b>:", reply_markup=get_cancel_menu_keyboard(), parse_mode="HTML")
    await state.set_state(AddSpotStates.waiting_spot_number)

@router.callback_query(F.data.startswith("address_"))
async def address_fallback(callback: CallbackQuery, state: FSMContext):
    cur = await state.get_state()
    if cur == AddSpotStates.waiting_address.state:
        return  # обработает sp_address
    await callback.answer("⚠️ Сначала нажмите «➕ Добавить место»", show_alert=True)
...
async def spot_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    if callback.data == "spot_confirm_no":
        await state.clear(); await callback.message.edit_text("❌ Отменено.")
        await callback.message.answer("Меню:", reply_markup=get_main_menu_keyboard(_adm(callback.from_user.id))); return
    data = await state.get_data()
    sdt = parse_datetime(data['start_date'], data['start_time_str'])
    ...
    if sdt < now_local():
        ...
        await state.clear();
        return
    # get_or_create — запоминает место
    spot_id = db.get_or_create_spot(data['supplier_id'], data['spot_number'], address=data.get('address'))
    # Проверка пересечений
    if db.check_slot_overlap(spot_id, sdt, edt):
        await callback.message.edit_text("❌ Слот пересекается с существующим!")
        await callback.message.answer("Меню:", reply_markup=get_main_menu_keyboard(_adm(callback.from_user.id)))
        await state.clear(); return
    # нормализация и проверки времени
    sdt = round_to_step(sdt, TIME_STEP_MINUTES)
    edt = round_to_step(edt, TIME_STEP_MINUTES)
    ok, msg = validate_interval(sdt, edt, now_tz(TIMEZONE), MIN_BOOKING_MINUTES, WORKING_HOURS_START, WORKING_HOURS_END)
    if not ok:
    ...
    await state.clear()
    await callback.message.edit_text(f"✅ <b>Слот добавлен!</b>\n\n🏠 {data['spot_number']}\n"
        f"📅 {format_datetime(sdt)} — {format_datetime(edt)}", parse_mode="HTML")
    await callback.message.answer("Меню:", reply_markup=get_main_menu_keyboard(_adm(callback.from_user.id)))
    for n in db.get_matching_notifications(spot_id, sdt, edt):
        try:
            await callback.bot.send_message(n['telegram_id'], f"🔔 Место {data['spot_number']}!")
