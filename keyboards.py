...
def get_available_slots_keyboard(slots):
    ...
    for slot in slots[:20]:
        ...
        text = f"🏠 {slot['spot_number']} | {date_text}"
...
def get_admin_slot_actions_keyboard(slot_id, is_booked):
    toggle_text = "🔓 Сделать свободным" if is_booked else "🔒 Сделать забронированным"
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=toggle_text, callback_data=f"adm_toggle_{slot_id}")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="admin_slots")]
    ])
